...
def main():
    ...
    map_label_list = [ner_remapping[a] for a in label_list]

    # Map that sends B-Xxx label to its I-Xxx counterpart
    b_to_i_label = []

    for idx, label in enumerate(map_label_list):
        if label.startswith("B-") and label.replace("B-", "I-") in map_label_list:
            b_to_i_label.append(map_label_list.index(label.replace("B-", "I-")))
        else:
    ...
    # Metrics
    metric = load_metric("accuracy",num_process=1, process_id=0)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            metric.add_batch(
                ...
                references=list(itertools.chain(*refs)),
                # predictions=preds,
                # references=refs,
