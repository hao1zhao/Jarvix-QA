...
def main():
    ...
    if args.dataset_name is not None:
        ...
        #raw_datasets = DatasetDict.from_json({'train':args.train_file, 'valid':args.validation_file})
        data_files = {}
        if args.train_file is not None:
    ...
    if raw_datasets["train"] is not None:
        ...
        #column_names = raw_datasets["train"]["rows"].column_names
        #print(f"column_names:{column_names}")
        #features = raw_datasets["train"].features
    else:
    ...
    def get_label_list(labels):
        ...
        label_list.sort()
        return label_list
    #print(f"label_column_name:{label_column_name}")
    #print(f"features[label_column_name]:{features[label_column_name]}")
    #print(type(features))
    ...
    with accelerator.main_process_first():
        #print(f"raw_datasets_0:{raw_datasets['train'][0]}")
        raw_datasets = raw_datasets.map(
            compute_sentence_boundaries_for_luke,
            batched=True,
            desc="Adding sentence boundaries",
        )
        #print(f"raw_datasets_1:{raw_datasets['train'][0]}")
        raw_datasets = raw_datasets.map(
            compute_entity_spans_for_luke,
            batched=True,
            desc="Adding sentence spans",
        )
        #print(f"raw_datasets_2:{raw_datasets['train'][0]}")
        ...
        #print(f"raw_datasets_3:{raw_datasets['train'][0]}")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            #print(f"label_column_name:{label_column_name}")
            #print(f"original_entity_spans:{original_entity_spans}")
